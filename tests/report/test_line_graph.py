...
class LineGraphTestCase(unittest.TestCase):
    ...
    def test_it_returns_a_value_of_zero_on_a_single_day(self):
        ...
        self.assertEqual(net_worth_values["times"], [datetime.datetime(2017, 1, 1, 0, 0)])
        self.assertEqual(net_worth_values["values"], [0])

    def test_it_returns_two_values_of_zero(self):
        line_graph = LineGraph(self.portfolio)
        net_worth_values = line_graph.net_worth_vs_time("2017-06-01", "2017-06-02")
        self.assertEqual(net_worth_values["times"], [datetime.datetime(2017, 6, 1, 0, 0), datetime.datetime(2017, 6, 2, 0, 0)])
        self.assertEqual(net_worth_values["values"], [0, 0])

    def test_it_returns_many_values_of_zero(self):
        line_graph = LineGraph(self.portfolio)
        net_worth_values = line_graph.net_worth_vs_time("2010-09-01", "2011-08-31")
        self.assertEqual(len(net_worth_values["times"]), 365)
        self.assertEqual(net_worth_values["values"], [0]*365)
    ...
    def test_it_returns_the_value_of_a_single_account(self):
        ...
        self.assertEqual(net_worth_values["values"], [0, 1000, 1000])
    ...
    def test_it_returns_the_value_of_two_accounts(self):
        ...
        self.assertEqual(net_worth_values["values"], [0, 0, 10, 10, -990, -990, -990])