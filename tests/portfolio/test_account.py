...
class AssetTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.account = Account("account name", "Bob Bobberson", "investment", AssetClass.CASH_EQUIVALENTS, "Rachel's Bank", AccountType.ASSET)
        self.liability = Account("account name", "Bob Bobberson", "investment", AssetClass.CASH_EQUIVALENTS, "Rachel's Bank", AccountType.LIABILITY)
    ...
    def test_it_throws_an_exception_if_a_string_is_passed_in_for_asset_class(self):
        invalid_account = Account("account name", "Bob Bobberson", "investment", AssetClass.CASH_EQUIVALENTS, "Rachel's Bank", "RANDOM")
    ...
    def test_it_throws_an_exception_if_a_string_is_passed_in_for_an_account_type(self):
        invalid_account = Account("account name", "Bob Bobberson", "investment", "Cash Equivalents", "Rachel's Bank", "RANDOM")
        self.assertRaises(AttributeError, invalid_account.account_type)
    ...
    def test_an_account_is_not_identical_to_one_with_a_different_name(self):
        different_account = Account("another name", "Bob Bobberson", "investment", AssetClass.CASH_EQUIVALENTS, "Rachel's Bank", AccountType.ASSET)
    ...
    def test_an_account_is_not_identical_to_one_with_a_different_owner(self):
        different_account = Account("account name", "Sam Sampson", "investment", AssetClass.CASH_EQUIVALENTS, "Rachel's Bank", AccountType.ASSET)
        self.assertFalse(self.account.is_identical_to(different_account))

    def test_an_account_is_not_identical_to_one_with_a_different_investment(self):
        different_account = Account("account name", "Bob Bobberson", "investment_2", AssetClass.CASH_EQUIVALENTS, "Rachel's Bank", AccountType.ASSET)
        self.assertFalse(self.account.is_identical_to(different_account))

    def test_an_account_is_not_identical_to_one_with_a_different_asset_class(self):
        different_account = Account("account name", "Bob Bobberson", "investment", AssetClass.EQUITIES, "Rachel's Bank", AccountType.ASSET)
        self.assertFalse(self.account.is_identical_to(different_account))

    def test_an_account_is_not_identical_to_one_with_a_different_institution(self):
        different_account = Account("account name", "Bob Bobberson", "investment", AssetClass.CASH_EQUIVALENTS, "Eric's Bank", AccountType.ASSET)
        self.assertFalse(self.account.is_identical_to(different_account))

    def test_an_account_is_not_identical_to_one_with_a_different_account_type(self):
        different_account = Account("account name", "Bob Bobberson", "investment", AssetClass.CASH_EQUIVALENTS, "Rachel's Bank", AccountType.LIABILITY)
        self.assertFalse(self.account.is_identical_to(different_account))
    ...
    def test_it_returns_a_row_for_a_balance_sheet(self):
        ...
        self.assertEqual(balance_sheet_row, ["\x1b[0;37;40m" + expected_date + "\x1b[0m", "Rachel's Bank","account name","investment","Bob Bobberson","Cash Equivalents","100"])
    ...
    def test_it_colors_the_date_red_if_it_is_in_the_future(self):
        ...
        self.account.import_snapshot(timestamp + date_difference, 100)
        balance_sheet_row = self.account.balance_sheet_row()
        self.assertEqual(balance_sheet_row, ["\x1b[1;31;40m" + expected_date + "\x1b[0m", "Rachel's Bank","account name","investment","Bob Bobberson","Cash Equivalents","0"])
    ...
    def test_it_colors_the_date_yellow_if_it_is_over_30_days_in_the_past(self):
        ...
        balance_sheet_row = self.account.balance_sheet_row()
        self.assertEqual(balance_sheet_row, ["\x1b[0;33;40m" + expected_date + "\x1b[0m", "Rachel's Bank","account name","investment","Bob Bobberson","Cash Equivalents","0"])
    ...
    def test_it_colors_the_date_pink_if_it_is_over_60_days_in_the_past(self):
        ...
        self.account.import_snapshot(timestamp - date_difference, 0)
        balance_sheet_row = self.account.balance_sheet_row()
        self.assertEqual(balance_sheet_row, ["\x1b[1;35;40m" + expected_date + "\x1b[0m", "Rachel's Bank","account name","investment","Bob Bobberson","Cash Equivalents","0"])
    ...
    def test_it_returns_a_row_for_a_liability_greater_than_one_week_old(self):
        ...
        self.liability.import_snapshot(timestamp - date_difference, 100)
        liabilities_row = self.liability.liabilities_row()
        self.assertEqual(liabilities_row, ["\x1b[0;30;41m" + expected_date + "\x1b[0m", "Rachel's Bank","account name","Bob Bobberson","100"])
